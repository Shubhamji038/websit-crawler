#!/usr/bin/env python3
"""
Python Web Crawler - A hakrawler-inspired web crawler for gathering URLs and JavaScript file locations.
"""
...
class PythonWebCrawler:
    def __init__(
        ...
        # Default user agent
        self.custom_headers.setdefault('User-Agent', 'Mozilla/5.0 (X11; Linux x86_64; rv:78.0) Gecko/20100101 Firefox/78.0')
    ...
    async def _fetch_page(self, url: str) -> Optional[str]:
        """Fetch page content."""
        try:
            timeout = ClientTimeout(total=self.timeout if self.timeout > 0 else None)
    ...
    async def _crawl_url(self, url: str, depth: int, source_url: str = ""):
        ...
        for link_url, source_type in links:
            ...
            if self._is_allowed_domain(url, link_url) and self._is_inside_path(url, link_url):
                # Add result
                result = Result(
                    url=link_url,
                    source=source_type,
...
async def main():
    parser = argparse.ArgumentParser(description='Python Web Crawler - hakrawler-inspired crawler')
    ...
    if sys.stdin.isatty():
        print("No URLs detected. Hint: echo 'https://example.com' | python python_webcrawler.py", file=sys.stderr)
        sys.exit(1)
    ...
    if output:
        ...
        print("No URLs were found. This usually happens when a domain is specified (https://example.com), but it redirects to a subdomain (https://www.example.com). The subdomain is not included in the scope, so no URLs are printed. In order to overcome this, either specify the final URL in the redirect chain or use the -subs option to include subdomains.", file=sys.stderr)
