...
class UnifiedWebCrawler:
    ...
    def quick_scan_combined(self, export_format=None, depth=None):
        ...
        filename = self.get_input("Filename (without extension)", f"scan_{url.replace('https://', '').replace('/', '_')}")
        ...
        for i, result in enumerate(crawler.results[:5], 1):
            source_tag = f"[{getattr(result, 'source', '')}] " if hasattr(result, 'source') and result.source else ""
    ...
    def advanced_scan_combined(self, export_format=None, depth=None):
        ...
        json_output = self.get_input("JSON output format? (y/n)", bool, (export_format == "json"))
        ...
        headers_input = self.get_input("Custom headers (format: 'Header: Value;;Header2: Value2')", str, "")
        ...
        # Export options
        if export_format:
            filename = self.get_input("Filename (without extension)", f"advanced_{url.replace('https://', '').replace('/', '_')}")
            self.export_results(filename, export_format)
        else:
        ...
        for i, result in enumerate(crawler.results[:10], 1):
            source_tag = f"[{getattr(result, 'source', '')}] " if hasattr(result, 'source') and result.source else ""
            where_tag = f"[{getattr(result, 'where', '')}] " if hasattr(result, 'where') and result.where else ""
